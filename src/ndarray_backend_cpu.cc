...
void NDIterator(const std::vector<int32_t>& shape, 
        const std::function<void(const std::vector<int32_t>&)>& cb) {
  size_t dim = shape.size();
  std::vector<int32_t> current_indices(dim, 0);
  while (true) {
    // Execute the callback with the current multi-dimensional index
    cb(current_indices);
    ...
    while (d >= 0) {
      current_indices[d]++;
      // If the current dimension's index is not overflowing, we're done incrementing.
      if (current_indices[d] < shape[d]) {
          break;
      }
      // Otherwise, reset it to 0 and "carry over" to the next (outer) dimension.
      current_indices[d] = 0;
      d--;
    }

    // If d < 0, it means we have carried over from the outermost dimension,
    // so we have visited all elements.
    if (d < 0) {
      break;
    }
  }
...
void Compact(const AlignedArray& a, AlignedArray* out, std::vector<int32_t> shape,
  ...
  NDIterator(shape, [&](const std::vector<int32_t>& current_indices) {
    size_t source_idx = offset;
    for (size_t i = 0; i < dim; ++i) {
      source_idx += current_indices[i] * strides[i];
    }
    out->ptr[compact_idx++] = a.ptr[source_idx];
  });
  /// END SOLUTION
}
...
void EwiseSetitem(const AlignedArray& a, AlignedArray* out, std::vector<int32_t> shape,
  ...
  NDIterator(shape, [&](const std::vector<int32_t>& current_indices) {
    ...
    for (size_t i = 0; i < dim; ++i) {
      dest_idx += current_indices[i] * strides[i];
    }
    out->ptr[dest_idx] = a.ptr[compact_idx++];
  });
  /// END SOLUTION
}
...
void MatmulTiled(const AlignedArray& a, const AlignedArray& b, AlignedArray* out, uint32_t m,
  ...
  const size_t tile_size_sq = TILE * TILE;
  for (size_t i = 0; i < m_tiles; i++) {
    for (size_t j = 0; j < p_tiles; j++) {
      float* out_tile_ptr = out->ptr + (i * p_tiles + j) * tile_size_sq;
      for (size_t l = 0; l < tile_size_sq; l++) {
        out_tile_ptr[l] = 0;
      }
      
      for (size_t k = 0; k < n_tiles; k++) {
        const float* a_tile_ptr = a.ptr + (i * n_tiles + k) * tile_size_sq;
        const float* b_tile_ptr = b.ptr + (k * p_tiles + j) * tile_size_sq;
