...
def verify_operation(result, expected_shape=None, expected_strides=None):
    """Verify that an operation produces expected results"""
    assert isinstance(result, nd.NDArray), "Result should be NDArray"
    if expected_shape is not None:
        assert result.shape == expected_shape, f"Expected shape {expected_shape}, got {result.shape}"
    if expected_strides is not None:
        assert result.strides == expected_strides, f"Expected strides {expected_strides}, got {result.strides}"
    return True


def test_ndarray_python_ops():
    """Test basic Python operations locally"""

    # Test reshape
    result1 = Rand(4, 4).reshape((2, 2, 4))
    verify_operation(result1, expected_shape=(2, 2, 4))
    
    result2 = Rand(2, 2, 4).reshape((4, 4))
    verify_operation(result2, expected_shape=(4, 4))
    
    # Test permute
    result3 = Rand(1, 3, 2, 1, 2).permute((1, 3, 2, 0, 4))
    verify_operation(result3, expected_shape=(3, 1, 2, 1, 2))
    
    # Test broadcast_to
    result4 = Rand(2, 1, 2).broadcast_to((2, 3, 2))
    verify_operation(result4, expected_shape=(2, 3, 2))
    
    result5 = Rand(1, 1, 2).broadcast_to((2, 2, 2))
    verify_operation(result5, expected_shape=(2, 2, 2))
    
    # Test getitem operations
    result6 = Rand(4, 4)[1, 0]
    result7 = Rand(4, 4)[1, 0:3]
    verify_operation(result7, expected_shape=(3,))
    
    result8 = Rand(4, 4)[1:3, 0:3]
    verify_operation(result8, expected_shape=(2, 3))
    
    result9 = Rand(4, 4, 4)[0:2, 1:3, 2:4]
    verify_operation(result9, expected_shape=(2, 2, 2))
    
    result10 = Rand(4, 4, 4)[0:4:2, :3, 2:]
    verify_operation(result10, expected_shape=(2, 3, 2))


def test_ndarray_cpu_compact_setitem():
    """Test CPU compact and setitem operations locally"""
    
    # Test compact operations
    result1 = Rand(4, 4, 4)[0:4:2, :3, 2:].compact()
    assert result1.is_compact(), "Array should be compact"
    
    result2 = Rand(1, 3, 2, 1, 2).permute((1, 3, 2, 0, 4)).compact()
    assert result2.is_compact(), "Array should be compact"
    
    result3 = Rand(1, 1, 2).broadcast_to((2, 2, 2)).compact()
    assert result3.is_compact(), "Array should be compact"
    
    result4 = Rand(4, 4).reshape((2, 2, 4)).compact()
    assert result4.is_compact(), "Array should be compact"
    
    # Test setitem operations
    A = Rand(4, 4)
    B = Rand(4, 4)
    start_ptr = A._handle.ptr()
    A[1, 0:3] = B[0, 1:4]
    end_ptr = A._handle.ptr()
    assert start_ptr == end_ptr, "Should modify in-place"
...
def test_ndarray_cpu_ops():
    """Test CPU element-wise operations locally"""
    
    # Test element-wise operations
    A, B = Rand(3, 3), Rand(3, 3)
    result1 = A * B
    verify_operation(result1, expected_shape=(3, 3))
    
    A, B = Rand(3, 3), Rand(3, 3)
    result2 = A / B
    verify_operation(result2, expected_shape=(3, 3))
    
    A, B = Rand(3, 3), Rand(3, 3)
    result3 = A == B
    verify_operation(result3, expected_shape=(3, 3))
    
    A, B = Rand(3, 3), Rand(3, 3)
    result4 = A >= B
    verify_operation(result4, expected_shape=(3, 3))
    
    A, B = Rand(3, 3), Rand(3, 3)
    result5 = A.maximum(B)
    verify_operation(result5, expected_shape=(3, 3))
    
    # Test scalar operations
    A = Rand(2, 2)
    result6 = A * 42.0
    verify_operation(result6, expected_shape=(2, 2))
    
    A = Rand(2, 2)
    result7 = A**2.0
    verify_operation(result7, expected_shape=(2, 2))
    
    A = Rand(2, 2)
    result8 = A / 42.0
    verify_operation(result8, expected_shape=(2, 2))
    
    A = Rand(5, 5)
    result9 = A.maximum(25.0)
    verify_operation(result9, expected_shape=(5, 5))
    
    A = Rand(10, 10)
    result10 = A == 10
    verify_operation(result10, expected_shape=(10, 10))
    
    A = Rand(5, 5)
    result11 = A > 50
    verify_operation(result11, expected_shape=(5, 5))
    
    # Test transcendental functions
    A = Rand(2, 2)
    result12 = A.log()
    verify_operation(result12, expected_shape=(2, 2))
    
    A = Rand(2, 2)
    result13 = A.tanh()
    verify_operation(result13, expected_shape=(2, 2))
    
    A = Rand(2, 2)
    result14 = (A / 100).exp()
    verify_operation(result14, expected_shape=(2, 2))


def test_ndarray_cpu_reductions():
    """Test CPU reduction operations locally"""
    # Test sum reductions
    result1 = Rand(4, 4).sum(axis=1)
    verify_operation(result1, expected_shape=(4, 1))
    
    result2 = Rand(4, 4, 4).sum(axis=1)
    verify_operation(result2, expected_shape=(4, 1, 4))
    
    result3 = Rand(4).sum(axis=0)
    verify_operation(result3, expected_shape=(1,))
    
    result4 = Rand(2, 2, 2, 4, 2).sum(axis=3)
    verify_operation(result4, expected_shape=(2, 2, 2, 1, 2))
    
    # Test max reductions
    result5 = Rand(4, 4).max(axis=1)
    verify_operation(result5, expected_shape=(4, 1))
    
    result6 = Rand(4, 4, 4).max(axis=1)
    verify_operation(result6, expected_shape=(4, 1, 4))
    
    result7 = Rand(4).max(axis=0)
    verify_operation(result7, expected_shape=(1,))
    
    result8 = Rand(2, 2, 2, 4, 2).max(axis=3)
    verify_operation(result8, expected_shape=(2, 2, 2, 1, 2))


def test_ndarray_cpu_matmul():
    """Test CPU matrix multiplication operations locally"""
    
    # Test basic matmul
    A, B = Rand(4, 4), Rand(4, 4)
    result1 = A @ B
    verify_operation(result1, expected_shape=(4, 4))
    ...
    result3 = A @ B
    verify_operation(result3, expected_shape=(73, 73))
    
    # Test tiled matmul
    for m, n, p in [(3, 2, 1), (3, 3, 3), (3, 4, 5)]:
        device = nd.cpu()
        if hasattr(device, "matmul_tiled") and hasattr(device, "__tile_size__"):
            t = device.__tile_size__
            A = Rand(m, n, t, t)
            B = Rand(n, p, t, t)
            C = nd.NDArray.make((m, p, t, t), device=nd.cpu())
            device.matmul_tiled(A._handle, B._handle, C._handle, m * t, n * t, p * t)
            verify_operation(C, expected_shape=(m, p, t, t))


def test_ndarray_cuda_compact_setitem():
    """Test CUDA compact and setitem operations locally"""
    if not nd.cuda().enabled():
        print("Skipping CUDA tests - no GPU available")
        return
    
    # Test compact operations
    result1 = RandC(4, 4, 4)[0:4:2, :3, 2:].compact()
    assert result1.is_compact(), "Array should be compact"
    
    result2 = RandC(1, 3, 2, 1, 2).permute((1, 3, 2, 0, 4)).compact()
    assert result2.is_compact(), "Array should be compact"
    
    result3 = RandC(1, 1, 2).broadcast_to((2, 2, 2)).compact()
    assert result3.is_compact(), "Array should be compact"
    
    result4 = RandC(4, 4).reshape((2, 2, 4)).compact()
    assert result4.is_compact(), "Array should be compact"
...
def test_ndarray_cuda_ops():
    """Test CUDA element-wise operations locally"""
    if not nd.cuda().enabled():
        print("Skipping CUDA tests - no GPU available")
        return
...
def test_ndarray_cuda_reductions():
    """Test CUDA reduction operations locally"""
    if not nd.cuda().enabled():
        print("Skipping CUDA tests - no GPU available")
        return
    
    # Test sum reductions
    result1 = RandC(4, 4).sum(axis=1)
    verify_operation(result1, expected_shape=(4, 1))
    
    result2 = RandC(4, 4, 4).sum(axis=1)
    verify_operation(result2, expected_shape=(4, 1, 4))
    
    result3 = RandC(4).sum(axis=0)
    verify_operation(result3, expected_shape=(1,))
    
    result4 = RandC(2, 2, 2, 4, 2).sum(axis=3)
    verify_operation(result4, expected_shape=(2, 2, 2, 1, 2))
    
    # Test max reductions
    result5 = RandC(4, 4).max(axis=1)
    verify_operation(result5, expected_shape=(4, 1))
    
    result6 = RandC(4, 4, 4).max(axis=1)
    verify_operation(result6, expected_shape=(4, 1, 4))
    
    result7 = RandC(4).max(axis=0)
    verify_operation(result7, expected_shape=(1,))
    
    result8 = RandC(2, 2, 2, 4, 2).max(axis=3)
    verify_operation(result8, expected_shape=(2, 2, 2, 1, 2))


def test_ndarray_cuda_matmul():
    """Test CUDA matrix multiplication operations locally"""
    if not nd.cuda().enabled():
        print("Skipping CUDA tests - no GPU available")
        return
    
    # Test various matrix multiplication sizes
    ...
    result4 = A @ B
    verify_operation(result4, expected_shape=(33, 33))
    
    A, B = RandC(73, 72), RandC(72, 71)
    result5 = A @ B
    verify_operation(result5, expected_shape=(73, 71))
    
    A, B = RandC(123, 125), RandC(125, 129)
    result6 = A @ B
    verify_operation(result6, expected_shape=(123, 129))
