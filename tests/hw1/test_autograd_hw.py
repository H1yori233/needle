...
def test_submit_forward():
    # Test power scalar operation
    x = np.array([[0.3, 2.5]])
    np.testing.assert_allclose(
        ndl.power_scalar(ndl.Tensor(x), scalar=3).numpy(),
        np.power(x, 3)
    )

    # Test tensor division
    a = np.array([[3.4, 2.35, 1.25], [0.45, 1.95, 2.55]])
    b = np.array([[4.9, 4.35, 4.1], [0.65, 0.7, 4.04]])
    np.testing.assert_allclose(
        ndl.divide(ndl.Tensor(a), ndl.Tensor(b)).numpy(),
        a / b
    )
    ...
    x = np.array([[1.4, 2.89]])
    np.testing.assert_allclose(
        ndl.divide_scalar(ndl.Tensor(x), scalar=7).numpy(),
        x / 7
    )

    # Test matrix multiplication
    a = np.array([[1.75, 1.75, 0.25], [4.95, 4.35, 0.3], [0.3, 1.4, 2.1]])
    b = np.array([[2.35, 2.2, 1.85], [7.85, 4.88, 2.6], [1.15, 0.25, 4.19]])
    np.testing.assert_allclose(
        ndl.matmul(ndl.Tensor(a), ndl.Tensor(b)).numpy(),
        a @ b
    )
    ...
    np.testing.assert_allclose(
        ndl.summation(ndl.Tensor(x)).numpy(),
        np.sum(x)
    )

    # Test summation with axis
    x = np.array([
        [5.05, 2.55, 1.0],
        [2.75, 3.7, 2.1],
        [0.1, 4.1, 3.3],
    ...
    np.testing.assert_allclose(
        ndl.summation(ndl.Tensor(x), axes=1).numpy(),
        np.sum(x, axis=1)
    )

    # Test broadcast operation
    x = np.array([[1.95, 3.85, -0.6]])
    np.testing.assert_allclose(
        ndl.broadcast_to(ndl.Tensor(x), shape=(3, 3, 3)).numpy(),
    ...
    np.testing.assert_allclose(
        ndl.reshape(ndl.Tensor(x), shape=(15,)).numpy(),
        x.reshape(15,)
    )
    ...
    np.testing.assert_allclose(
        ndl.reshape(ndl.Tensor(x), shape=(2, 3, 4)).numpy(),
        x.reshape(2, 3, 4)
    )

    # Test negation
    x = np.array([[1.45, 0.55]])
    np.testing.assert_allclose(
        ndl.negate(ndl.Tensor(x)).numpy(),
        -x
    )

    # Test transpose with axes
    x = np.array([[[3.45]], [[2.54]], [[1.91]]])
    np.testing.assert_allclose(
        ndl.transpose(ndl.Tensor(x), axes=(0, 1)).numpy(),
        np.transpose(x, axes=(1, 0, 2))
    )

    # Test simple transpose
    x = np.array([[4.45, 2.15], [1.89, 1.21], [6.15, 2.42]])
    np.testing.assert_allclose(
        ndl.transpose(ndl.Tensor(x)).numpy(),
        x.T
    )
    ...
    b = np.array([[2, 0]])
    np.testing.assert_allclose(
        ndl.power(ndl.Tensor(a), ndl.Tensor(b)).numpy(),
        np.power(a, b)
    )
...
def test_submit_backward():
    ...
    # Test matmul gradient with broadcasting
    assert gradient_check(
        ndl.matmul,
        ndl.Tensor(np.random.randn(2, 4)),
    ...
    # Test matmul gradient 3D tensors
    assert gradient_check(
        ndl.matmul,
    ...
    # Test matmul gradient with batch dimensions
    assert gradient_check(
        ndl.matmul,
        ndl.Tensor(np.random.randn(2, 4)),
    ...
    assert gradient_check(ndl.negate, ndl.Tensor(np.random.randn(1, 4, 2))) == True
    ...
    # Test summation gradients
    assert gradient_check(ndl.summation, ndl.Tensor(np.random.randn(3, 2, 1))) == True
    
    assert gradient_check(
        ndl.summation, ndl.Tensor(np.random.randn(3, 6)), axes=(1,)
    ) == True
    ...
    # Test log and exp gradients
    assert gradient_check(ndl.log, ndl.Tensor(10 + np.random.rand(5, 4))) == True
    
    assert gradient_check(ndl.exp, ndl.Tensor(1.5 + np.random.rand(5, 4))) == True
...
def test_submit_topo_sort():
    ...
    # Verify we got a valid topological order
    assert len(topo_order) > 0, "Topological order should not be empty"
    print("Test case 1 - Topo order length:", len(topo_order))
    print("Test case 1 - Final result shape:", topo_order[-1].shape)
    ...
    # Verify we got a valid topological order
    assert len(topo_order2) > 0, "Topological order should not be empty"
    print("Test case 2 - Topo order length:", len(topo_order2))
    print("Test case 2 - Final result shape:", topo_order2[-1].shape)
    ...
    # Verify we got a valid topological order
    assert len(topo_order3) > 0, "Topological order should not be empty"
    print("Test case 3 - Topo order length:", len(topo_order3))
    print("Test case 3 - Final result shape:", topo_order3[-1].shape)
...
def test_submit_compute_gradient():
    # Test case 1 - Simple gradient computation
    ...
    assert grads[0].shape == a.shape, f"Gradient shape mismatch for a: {grads[0].shape} vs {a.shape}"
    assert grads[1].shape == b.shape, f"Gradient shape mismatch for b: {grads[1].shape} vs {b.shape}"
    assert grads[2].shape == c.shape, f"Gradient shape mismatch for c: {grads[2].shape} vs {c.shape}"
    print("Test case 1 gradients:", grads)
    ...
    assert grads[1].shape == b.shape, f"Gradient shape mismatch for b: {grads[1].shape} vs {b.shape}"
    assert grads[2].shape == c.shape, f"Gradient shape mismatch for c: {grads[2].shape} vs {c.shape}"
    print("Test case 2 gradients:", grads)

    # Test case 3 - Second-order gradients
    x2 = ndl.Tensor(3)
    x3 = ndl.Tensor(2)
    y = x2 * x2 - x2 * x3
    y.backward()
    ...
    y_result = y.numpy()
    grad_x2_result = grad_x2.numpy()
    grad_x3_result = grad_x3.numpy()
    grad_x2_x2_result = grad_x2_x2.numpy()
    
    print("Function value y =", y_result)
    print("Gradient wrt x2 =", grad_x2_result)
    print("Gradient wrt x3 =", grad_x3_result)
    print("Second-order gradient x2_x2 =", grad_x2_x2_result)
    
    # Basic sanity checks
    assert y_result == 3, f"Expected y=3, got {y_result}"
    assert grad_x2_result == 4, f"Expected grad_x2=4, got {grad_x2_result}"
    assert grad_x3_result == -3, f"Expected grad_x3=-3, got {grad_x3_result}"
...
def test_submit_softmax_loss_ndl():
    # Test log gradient
    np.random.seed(0)
    log_gradient_test = gradient_check(ndl.log, ndl.Tensor(1 + np.random.rand(5, 4)))
    assert log_gradient_test == True, "Log gradient check failed"
    print("Log gradient check passed")
    ...
    # Test softmax loss with zeros
    loss_zeros = softmax_loss(
        ndl.Tensor(np.zeros((y.shape[0], 10)).astype(np.float32)), y
    ).numpy()
    
    # Loss should be -log(1/10) ≈ 2.3026 for uniform predictions
    expected_loss_zeros = -np.log(0.1)
    np.testing.assert_allclose(loss_zeros, expected_loss_zeros, rtol=1e-5)
    print(f"Softmax loss with zeros: {loss_zeros}")
    
    # Test softmax loss with random weights
    np.random.seed(0)
    loss_random = softmax_loss(
        ndl.Tensor(np.random.randn(y.shape[0], 10).astype(np.float32)), y
    ).numpy()
    
    # Random loss should be different from uniform case
    assert loss_random != loss_zeros, "Random loss should differ from zeros case"
    print(f"Softmax loss with random weights: {loss_random}")
...
def test_submit_nn_epoch_ndl():
    X, y = parse_mnist(
        "data/train-images-idx3-ubyte.gz", "data/train-labels-idx1-ubyte.gz"
    )
    
    # Test case 1: Small batch training
    np.random.seed(1)
    W1 = ndl.Tensor(np.random.randn(X.shape[1], 100).astype(np.float32) / np.sqrt(100))
    W2 = ndl.Tensor(np.random.randn(100, 10).astype(np.float32) / np.sqrt(10))
    W1, W2 = nn_epoch(X[:100], y[:100], W1, W2, lr=0.1, batch=100)

    w1_norm = np.linalg.norm(W1.numpy())
    w2_norm = np.linalg.norm(W2.numpy())
    
    # Check that weights have reasonable norms after training
    assert w1_norm > 0, "W1 norm should be positive"
    assert w2_norm > 0, "W2 norm should be positive"
    print(f"Test case 1 - W1 norm: {w1_norm}")
    print(f"Test case 1 - W2 norm: {w2_norm}")

    # Test case 2: Full dataset training
    np.random.seed(1)
    W1 = ndl.Tensor(np.random.randn(X.shape[1], 100).astype(np.float32) / np.sqrt(100))
    W2 = ndl.Tensor(np.random.randn(100, 10).astype(np.float32) / np.sqrt(10))
    W1, W2 = nn_epoch(X, y, W1, W2, lr=0.2, batch=100)

    w1_norm_full = np.linalg.norm(W1.numpy())
    w2_norm_full = np.linalg.norm(W2.numpy())
    loss_err_result = loss_err(ndl.Tensor(np.maximum(X @ W1.numpy(), 0)) @ W2, y)
    
    # Check that weights have reasonable norms after full training
    assert w1_norm_full > 0, "W1 norm should be positive"
    assert w2_norm_full > 0, "W2 norm should be positive"
    assert 0 <= loss_err_result <= 1, f"Loss/error should be between 0 and 1, got {loss_err_result}"
    
    print(f"Test case 2 - W1 norm: {w1_norm_full}")
    print(f"Test case 2 - W2 norm: {w2_norm_full}")
    print(f"Test case 2 - Loss/error: {loss_err_result}")
