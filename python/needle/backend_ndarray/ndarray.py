...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        if not self.is_compact():
          raise ValueError("Reshape can only be called on a compact array. Call .compact() first.")
    ...
    def permute(self, new_axes):
        ...
        assert sorted(list(new_axes)) == list(range(self.ndim))
        ...
        for i in range(self.ndim):
          if self.shape[i] != 1 and self.shape[i] != new_shape[i]:
            raise AssertionError(
              f"new_shape: {new_shape[i]} != {self.shape[i]} for {i} where {self.shape[i]} != 1"
            )

          if self.shape[i] == 1: # for those broadcasted dim, set stride to 0
    ...
    def __getitem__(self, idxs):
        ...
        for i, sl in enumerate(idxs):
          ...
          # e.g. sl = slice(1, 5, 2) -> shape = 3 // 2 + 1 = 2
          #    sl = slice(1, 6, 2) -> shape = 4 // 2 + 1 = 3
          new_shape.append((sl.stop - sl.start - 1) // sl.step + 1)
